module.exports = {
  ...
  BOARD_JOIN_USER_SELECT_ONE_WHERE_ID:
    "SELECT b.id AS 'board_id', b.title, b.contents, bc.description AS 'category', u.id AS 'user_id', u.username, b.create_date, b.view_count FROM board AS b, board_category AS bc, users AS u WHERE b.user_id = u.id AND b.category_id = bc.id AND b.id = ?;",
  BOARD_JOIN_USER_JOIN_CATEGORY_ALL_WHERE_CATEGORY_ID_LIMIT_OFFSET:
    "SELECT b.id, b.title, u.username, b.create_date, b.view_count FROM board b, board_category bc, users u WHERE b.category_id = bc.id AND b.user_id = u.id AND bc.id = ? ORDER BY b.id DESC LIMIT ?,?;",
